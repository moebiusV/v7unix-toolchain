...
void build(int16_t op)
{
	...
	case STAR:
		...
		*cp++ = block(STAR, decref(t1), p1->u.tnode.subsp, p1->u.tnode.strp, p1, NULL);
		return;

	case AMPER:
		if (p1->u.tnode.op==NAME || p1->u.tnode.op==STAR) {
			*cp++ = block(op,incref(t1),p1->u.tnode.subsp,p1->u.tnode.strp,p1, NULL);
			return;
		}
	...
	if ((dope&BINARY)==0) {
		if (op==ITOF)
			...
			t1 = INT;
		if (!fold(op, p1, 0))
			*cp++ = block(op,t1,p1->u.tnode.subsp,p1->u.tnode.strp,p1, NULL);
		return;
	}
	cvn = 0;
...
struct node * nblock(struct node *ads)
{
	...
	ds = ads;
	return(block(NAME, ds->u.hshtab.htype, ds->u.hshtab.hsubsp, ds->u.hshtab.hstrp, ds, NULL));
}
