...
int16_t	peeksym = -1;
int16_t	line = 1;
struct node funcblk = { { .tnode = { NAME, 0, NULL, NULL, NULL, NULL } } };
...
{
	register char *np;
	register int16_t c, base;
	int16_t expseen, sym, ndigit;
	char *nsyn;
	int16_t maxdigit;
...
int16_t mapch(int16_t ac)
{
	register int16_t a, c, n;
	static int16_t mpeek;
...
advanc:
	...
	case STRING:
		...
		*cp++ = block(NAME, ARRAY+CHAR, &nchstr, NULL, cs, NULL);
