...
struct	node {
	...
	union {
		...
		struct { int16_t value; long double fvalue; }	ftconst;
...
extern void pdp11_double(long double d, uint16_t w[4]);
extern void pdp11_float(long double d, uint16_t w[2]);
extern void pdp11_long(int32_t v, uint16_t w[2]);
extern long double v7_atof(const char *p);
