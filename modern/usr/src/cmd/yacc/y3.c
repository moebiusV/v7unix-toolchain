...
int output(void){ /* print the output for the states */
	...
	for(i=0;i<nstate;++i) /* SLOOP() in v7 */ { /* output the stuff for state i */
		...
		for(u=wsets;u<cwp;++u) /* WSLOOP() in v7 */{
			...
			if( c<=0 ){ /* reduction */
				lastred = -c;
				for(k=1;k<=ntokens;++k) /* TLOOP() in v7 */{
					if( bit(u->ws.lset,k) ){
...
int apack(int *p, int n){ /* pack state i from temp1 into amem */
	...
	for( rr=amem; rr<=r; ++rr,++off ){  /* try rr */
		...
		for( qq=rr,pp=p; pp<=q; ++pp,++qq ){
			if( *pp ){
				if( qq > r ) error( "action table overflow" , 0);
				if( qq>memp ) memp = qq;
				*qq = *pp;
				}
			}
	...
	error("no space in action table" , 0);
	/* NOTREACHED */
	}

int go2out(void){ /* output the gotos for the nontermninals */
...
int g2debug = 0;
int go2gen(int c){ /* output the gotos for nonterminal c */
	...
	if( g2debug && foutput!=NULL ){
		...
		for(i=0;i<=nnonter;++i) /* NTLOOP() in v7 */ if( temp1[i] ) fprintf( foutput, "%s ", nontrst[i].name);
		fprintf( foutput, "\n");
		}
...
void wract(int i){ /* output state i */
	...
	for(j=1;j<=ntokens;++j) /* TLOOP() in v7 */{
		...
		tred = -temp1[j];
		levprd[tred] |= REDFLAG;
		for(p=1;p<=ntokens;++p) /* TLOOP() in v7 */{
	...
	for(p=1;p<=ntokens;++p) /* TLOOP() in v7 */ if( temp1[p]+lastred == 0 )temp1[p]=0;

	wrstate(i);
	defact[i] = lastred;

	flag = 0;
	for(p0=1;p0<=ntokens;++p0) /* TLOOP() in v7 */{
		if( (p1=temp1[p0])!=0 ) {
			if( p1 < 0 ){
				p1 = -p1;
				goto exc;
...
void wrstate(int i){ /* writes state i */
	...
	fprintf( foutput, "\nstate %d\n",i);
	qq=pstate[i+1];for(pp=pstate[i];pp<qq;++pp) /* ITMLOOP() in v7 */ fprintf( foutput, "\t%s\n", writem(pp->pitem));
	if( tystate[i] == MUSTLOOKAHEAD ){
		/* print out empty productions in closure */
		for(u=wsets+(pstate[i+1]-pstate[i]);u<cwp;++u) /* WSLOOP() in v7 */{
	...
	for(j0=1;j0<=ntokens;++j0) /* TLOOP() in v7 */ if( (j1=temp1[j0]) != 0 ){
		fprintf( foutput, "\n\t%s  ", symnam(j0) );
...
int wdef(char *s, int n){ /* output a definition of s to the value n */
