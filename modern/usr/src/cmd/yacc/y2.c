...
/* token codes returned by gettok() (the hand-written lexer in this file) */
enum {
	IDENTIFIER = 257,
	MARK = 258,
	TERM = 259,
	...
	PREC = 263,
	LCURLY = 264,
	C_IDENTIFIER = 265,  /* name followed by colon */
	NUMBER = 266,
	START = 267,
	TYPEDEF = 268,
	TYPENAME = 269,
...
/* output/temp file names (declared extern in dextern) */
const char *tempname = "yacc.tmp";
const char *actname = "yacc.acts";
const char *ofile = "y.tab.c";
const char *fileu = "y.output";
const char *filed = "y.tab.h";
		...
		case TERM:
			...
			for(;;) {
				...
				case IDENTIFIER:
					...
					if( ty ){
						if( toktype(toklev[j]) ) error( "redeclaration of type of %s", tokname );
						settype(&toklev[j],ty);
						}
					if( (t=gettok()) == NUMBER ){
	...
	more_rule:
		...
		if( t == '=' ){
			...
			if( (t=gettok()) == IDENTIFIER ){
				...
				sprintf( aname, "$$%d", nprod );
				j = chfind(1,aname);  /* make it a nonterminal */
		...
		*mem++ = -nprod;
		...
		if( ntypes && !(levprd[nprod]&ACTFLAG) && nontrst[*prdptr[nprod]-NTBASE].tvalue ){
			...
			else tempty = toktype( toklev[tempty] );
...
int defout(void){ /* write out the defines (at the end of the declaration section) */
