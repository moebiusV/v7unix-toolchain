...
comline:  START
	...
	| START namelist deplist shellist {
		...
		lp = ckalloc(sizeof(struct lineblock)) /* ALLOC(lineblock) in v7 */;
		lp->nxtlineblock = NULL;
		lp->depp = $3;
		lp->shp = $4;

		if(! strcmp(leftp->namep, ".SUFFIXES") && $3==0)
			leftp->linep = 0;
...
dlist:  sepchar	{ prevdep = 0;  $$ = 0; }
	| dlist NAME	{
			  pp = ckalloc(sizeof(struct depblock)) /* ALLOC(depblock) in v7 */;
			  pp->nxtdepblock = NULL;
			  pp->depname = $2;
			  if(prevdep == 0) $$ = pp;
...
sp = ckalloc(sizeof(struct shblock)) /* ALLOC(shblock) in v7 */;
sp->nxtshblock = NULL;
