...
int16_t mactop,fretop;
char *instack[MAXFRE],*bufstack[MAXFRE],*endbuf[MAXFRE];

int16_t plvl;	/* parenthesis level during scan for macro actuals */
int16_t maclin;	/* line number of macro call requiring actuals */
char *macfil;	/* file name of macro call requiring actuals */
char *macnam;	/* name of macro requiring actuals */
int16_t maclvl;	/* # calls since last decrease in nesting level */
...
char * copy(char *s);
int yyparse(void);	/* host fix: V7 called it undeclared (K&R implicit int) */
void pperror(char *s, ...);
void ppwarn(char *s, ...);
void yyerror(const char *s);
struct symtab * lookup(char *namep, int16_t enterf);
struct symtab * slookup(char *p1, char *p2, int16_t enterf);
	...
	for (;;) {
		if (mactop>inctop[ifno]) {/* retrieve hunk of pushed-back macro text */
			...
			if (ifno==0) {/* end of input */
				if (plvl!=0) {
					int16_t n=plvl,tlin=lineno[ifno]; char *tfil=fnames[ifno];
...
char * skipbl(char *p){/* get next non-blank token */
...
int16_t equfrm(char *a, char *p1, char *p2){
	register char c; int16_t flag;
	c= *p2; *p2='\0';
	flag=strcmp(a,p1); *p2=c; return(flag==SAME);
}

char * dodef(char *p){/* process '#define' */
...
/* Host port: V7's pperror/ppwarn/yyerror were K&R "generic argument" functions
   (format string + up to two args passed through to fprintf).  On the PDP-11
   an int and a char* are both 16 bits, so the two arguments could carry either;
   on a 64-bit host that truncates pointers.  Rebuilt as real varargs. */
void pperror(char *s, ...){
	va_list ap;
...
void ppwarn(char *s, ...){
	...
	exfail = -1;
	va_start(ap, s);
	/* re-format into a single string so the varargs can be forwarded */
	{
		char buf[512];
...
struct symtab * lookup(char *namep, int16_t enterf)
{
	...
	register int16_t c, i; int16_t around;
	register struct symtab *sp;
...
int16_t main(int16_t argc, char *argv[])
{
	...
	fout = stdout;	/* host fix: glibc stdout is a runtime value, not a constant initializer */
