...
void execexp(STRING s, intptr_t f);
	...
	if( (t=argt) && execbrk==0
		...
		case TCOM:
			...
			if( argn && (flags&noexec)==0
				...
				case SYSTIMES:
					{
					struct tms	tb; times(&tb);	/* v7: L_INT t[4]; times(t); */
					prt(tb.tms_cutime); blank(); prt(tb.tms_cstime); newline();
					}
					break;
...
*/
		...
		case TFORK:
			...
			if( parent
				...
				/* pipe in or out */
				if( treeflgs&FPIN
				){	rename_sh(pf1[INPIPE],0);
					close(pf1[OTPIPE]);
				;}
				if( treeflgs&FPOU
				){	rename_sh(pf2[OTPIPE],1);
					close(pf2[INPIPE]);
				;}

				/* default std input for & */
				if( treeflgs&FINT && ioset==0
				){	rename_sh(chkopen(devnull),0);
				;}
		...
		case TIF:
			...
			} else {	execute(((IFPTR)t)->eltre,execflg, 0, 0);
			;}
			break;
