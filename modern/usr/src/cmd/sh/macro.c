...
retry:
	...
	if( d==DOLLAR
		...
		if( (c=readc(), dolchar(c))
			...
			if( letter(c)
			){	argp=(STRING)(intptr_t)relstak();
				while( alphanum(c) ){ pushstak(c); c=readc() ;}
				zerostak();
				n=lookup(absstak((int)(intptr_t)argp)); setstak((int)(intptr_t)argp);
				...
				;}
				c -= '0';
				v=((c==0) ? cmdadr : (c<=dolc) ? dolv[c] : (dolg=0, (STRING)0));
			} else if ( c=='$'
			){	v=pidadr;
			} else if ( c=='!'
			){	v=pcsadr;
			...
			if( bra
					...
					;}
					argp=absstak((int)(intptr_t)argp);
				;}
