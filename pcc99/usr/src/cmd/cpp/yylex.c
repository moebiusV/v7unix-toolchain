...
int16_t tobinary(char *st, int16_t b);

int16_t yylex(void){
	static int16_t ifdef=0;
	static char *op2[]={"||",  "&&" , ">>", "<<", ">=", "<=", "!=", "=="};
	static int16_t  val2[]={OROR, ANDAND,  RS,   LS,   GE,   LE,   NE,   EQ};
	static char *opc="b\bt\tn\nf\fr\r\\\\";
	extern char fastab[];
	extern char *outp,*inp,*newp; extern int16_t flslvl;
	register char savc, *s; char *skipbl(); int16_t val;
	register char **p2;
	struct symtab {
		char *name;
		char *value;
