...
int16_t tbitset;  /* size of lookahead sets */
struct looksets lkst [ LSETSIZE ];
int16_t nlset = 0; /* next lookahead set index */
int16_t nolook = 0; /* flag to suppress lookahead computations */
...
int16_t nstate = 0;		/* number of states */
struct item *pstate[NSTATES+2];	/* pointers to the descriptions of the states */
int16_t tystate[NSTATES];	/* contains type information about the states */
int16_t indgo[NSTATES];		/* index to the stored goto table */
int16_t tstates[ NTERMS ]; /* states generated by terminal gotos */
int16_t ntstates[ NNONTERM ]; /* states generated by nonterminal gotos */
int16_t mstates[ NSTATES ]; /* chain of overflows of term/nonterm generation lists  */

	/* storage for the actions in the parser */

int16_t amem[ACTSIZE];	/* action table storage */
int16_t *memp = amem;	/* next free action table position */

	/* other storage areas */

int16_t temp1[TEMPSIZE]; /* temporary storage, indexed by terms + ntokens or states */
int16_t lineno= 1; /* current input line number */
int16_t fatfl = 1;  	/* if on, error is fatal */
int16_t nerrors = 0;	/* number of errors */

	/* storage for information about the nonterminals */

int16_t **pres[NNONTERM+2];  /* vector of pointers to productions yielding each nonterminal */
struct looksets *pfirst[NNONTERM+2];  /* vector of pointers to first sets for each nonterminal */
int16_t pempty[NNONTERM+1];  /* vector of nonterminals nontrivially deriving e */
...
int16_t others(void){ /* put out other arrays, copy the parsers */
	register int16_t c, i, j;
...
# define ISIZE 400
char * writem(int16_t *pp){ /* creates output string for item pointed to by pp */
	int16_t i,*p;
...
char * symnam(int16_t i){ /* return a pointer to the name of symbol i */
	char *cp;
...
int16_t zzclose = 0;
int16_t zzsrconf = 0;
int16_t * zzmemsz = mem0;
int16_t zzrrconf = 0;

int16_t summary(void){ /* output the summary on the tty */
...
/* VARARGS1 */
void error(char *s, int16_t a1){ /* write out error comment */
...
int16_t aryfil(int16_t *v, int16_t n, int16_t c){ /* set elements 0 through n-1 to c */
	int16_t i;
	for( i=0; i<n; ++i ) v[i] = c;
	}
...
int16_t cpres(void){ /* compute an array with the beginnings of  productions yielding given nonterminals
	The array pres points to these lists */
	/* the array pyield has the lists: the total size is only NPROD+1 */
	register int16_t **pmem;
	register int16_t c, j, i;
	static int16_t * pyield[NPROD];
...
void cpfir(void){
	...
	register int16_t *p, **s, i, **t, ch, changes;
...
int16_t state(int16_t c){ /* sorts last state,and sees if it equals earlier ones. returns state number */
	int16_t size1,size2;
	register int16_t i;
...
int16_t pidebug = 0; /* debugging flag for putitem */
int16_t putitem(int16_t *ptr, struct looksets *lptr){
	...
	pstate[nstate+1] = ++j;
	if( (int16_t *)j > zzmemsz ){
		zzmemsz = (int16_t *)j;
		if( zzmemsz >=  &mem0[MEMSIZE] ) error( "out of state space" , 0);
		}
	}

int16_t cempty(void){ /* mark nonterminals which derive the empty string */
...
int16_t gsdebug = 0;
int16_t stagen(void){ /* generate the states */
...
int16_t cldebug = 0; /* debugging flag for closure */
int16_t closure(int16_t i){ /* generate the closure of state i */
