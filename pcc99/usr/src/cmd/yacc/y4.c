...
int16_t * ggreed = lkst[0].lset;
int16_t * pgo = wsets[0].ws.lset;
int16_t *yypgo = &nontrst[0].tvalue;

int16_t maxspr = 0;  /* maximum spread of any entry */
int16_t maxoff = 0;  /* maximum offset into a array */
int16_t *pmem = mem;
int16_t *maxa;
# define NOMORE -1000
...
int16_t gin(int16_t i){
	...
	for( p=a; p < &a[ACTSIZE]; ++p ){
		...
		for( r=q1; r<q2; r+=2 ){
			...
			if( *s ) goto nextgp;
			if( s > maxa ){
				if( (maxa=s) > &a[ACTSIZE] ) error( "a array overflow" , 0);
				}
			}
		...
		*p = *q2;
		if( p > maxa ){
			if( (maxa=p) > &a[ACTSIZE] ) error( "a array overflow" , 0);
			}
		for( r=q1; r<q2; r+=2 ){
			s = p + *r + 1;
			*s = r[1];
...
void stin(int16_t i){
	register int16_t *r, *s, n, flag, j, *q1, *q2;
	...
	for( n= -maxoff; n<ACTSIZE; ++n ){
		...
		for( r = q1; r < q2; r += 2 ){
			if( (s = *r + n + a ) >= &a[ACTSIZE] ) error( "out of space in optimizer a array" , 0);
			if( s > maxa ) maxa = s;
			if( *s != 0 && *s != r[1] ) error( "clobber of a array, pos'n %d", s-a );
			*s = r[1];
			}
		pa[i] = n;
...
int16_t nxti(void){ /* finds the next i */
	register int16_t i, max, maxi;
...
int16_t aoutput(void){ /* this version is for C */
...
int16_t gtnm(void){
	...
	*pmem++ = s*val;
	if( pmem > &mem[MEMSIZE] ) error( "out of space" , 0);
	return( c );
