...
int16_t syslook(STRING w, SYSTAB syswds)
{
	REG CHAR	first;
	REG STRING	s;
	REG SYSPTR	syscan;
...
int16_t assign(NAMPTR n, STRING v);
...
int16_t setlist(ARGPTR arg, INT xp)
{
	WHILE arg
...
int16_t replace(STRING *a, STRING v)
{
	free(*a); *a=make(v);
}

int16_t dfault(NAMPTR n, STRING v)
{
	IF n->namval==0
	THEN	assign(n,v)
	FI
...
INT readvar(STRING *names)
{
	...
	IF eof THEN rc=1 FI
	lseek(0, (int32_t)(f->fnxt-f->fend), 1);
	pop();
	return(rc);
}

int16_t assnum(STRING *p, INT i)
{
