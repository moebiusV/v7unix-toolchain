...
int16_t branch(STRING s, INT ins);
int16_t doubl(int16_t a, int16_t b);
void paddr(STRING s, INT a);

int16_t printins(int16_t f, int16_t idsp, INT ins)
{
	INT		byte;
	REG OPTAB	p;
	...
	switch (p->itype) {
		...
		BEGIN
		   INT		indir;
		   REG INT	w;
		   aprintf("%8t%s", systab[ins &= 077].sname);
