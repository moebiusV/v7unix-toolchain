...
int16_t eqstr(STRING s1, STRING s2)
{
	REG STRING	 es1;
	es1 = s1+8;
	WHILE *s1++ == *s2
...
int16_t charpos(void)
{	return(printptr-printbuf);
}

int16_t flushbuf(void)
{	IF printptr!=printbuf
	THEN printc(EOR);
	FI
}

int16_t convert(STRING *cp);
int16_t printdate(L_INT tvec);
int16_t printdbl(INT lx, INT ly, char fmat, int16_t base);
int16_t printnum(INT n, int16_t fmat, int16_t base);
int16_t printoct(L_INT o, INT s);

int16_t aprintf(fmat, a1)
	STRING		fmat;
	STRING		*a1;
{
	STRING		fptr, s;
	...
	DO  IF c!='%'
	    ...
	    ELSE IF *fptr=='-' THEN adj='l'; fptr++; ELSE adj='r'; FI
		 ...
		 digitptr=digits;
		 dptr=(L_INT *)vptr; rptr=(L_REAL *)vptr; lx = *dptr; x = *vptr++;
		 s=0;
		 switch (c = *fptr++) {
		    ...
		    case 'D':
		    case 'U':
			printdbl((INT)(lx>>16),(INT)lx,c,10); vptr++; break;
		    case 'O':
			printoct(lx,0); vptr++; break;
		    case 'Q':
			printoct(lx,-1); vptr++; break;
		    case 'X':
			printdbl((INT)(lx>>16),(INT)lx,'x',16); vptr++; break;
		    case 'c':
			printc(x); break;
		    case 's':
			s=(STRING)x; break;
...
int16_t prints(char *s)
{	aprintf("%s",s);
}

int16_t newline(void)
{
	printc(EOR);
}
...
int16_t printdbl(INT lx, INT ly, char fmat, int16_t base)
{	int16_t digs[20]; int16_t *dptr; char k;
	L_REAL f ,g; int32_t q;
	dptr=digs;
	IF fmat!='D'
