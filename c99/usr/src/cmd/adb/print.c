...
int16_t printfregs(int16_t longpr);
int16_t printmap(STRING s, MAP *amap);
int16_t printpc(void);
int16_t printregs(void);
int16_t sigprint(void);

void printtrace(int16_t modif)
{
	...
	switch (modif) {
		...
		DO	chkerr();
			...
			aprintf("%.8s(", symbol.symc);
			argp = frame+4;
			IF --narg >= 0
			THEN	aprintf("%o", get(argp, DSP));
			FI
			WHILE --narg >= 0
			DO	argp += 2;
				aprintf(",%o", get(argp, DSP));
			OD
			prints(")\n");
			...
			THEN WHILE localsym(frame)
				...
				aprintf("%8t%.8s:%10t", symbol.symc);
				IF errflg THEN prints("?\n"); errflg=0; ELSE aprintf("%o\n",word); FI
			     OD
			FI
		...
		DO chkerr();
		   ...
		   IF modif=='A'
		   THEN aprintf("%8O:%8t%-8o,%-8o,%-8o",frame,stat,dynam,link);
		   FI
		   ...
		   ELSE aprintf("%8t");
			...
			printc('"');
		   FI
		   limit=4; i=6; aprintf("%24targs:%8t");
		   WHILE limit--
		   DO aprintf("%8t%o",get(frame+i,DSP)); i += 2; OD
		   printc(EOR);
...
int16_t printmap(STRING s, MAP *amap)
{
	...
	aprintf("b1 = %-16Q",amap->b1);
	aprintf("e1 = %-16Q",amap->e1);
	aprintf("f1 = %-16Q",amap->f1);
	aprintf("\nb2 = %-16Q",amap->b2);
	aprintf("e2 = %-16Q",amap->e2);
	aprintf("f2 = %-16Q",amap->f2);
	printc(EOR);
}
...
int16_t printfregs(int16_t longpr)
{
	...
	aprintf("fpsr	%o\n", ((struct adb_fpsave *)corhdr)->fpsr);
	FOR i=0; i<FRMAX; i++
	DO	IF ((struct adb_fpsave *)corhdr)->fpsr&FD ORF longpr	/* long mode */
		THEN	f = ((struct adb_lfsave *)corhdr)->Lfr[frnames[i]];
		ELSE	f = ((struct adb_fpsave *)corhdr)->Sfr[frnames[i]];
		FI
		aprintf("fr%-8d%-32.18f\n", i, f);
	OD
}
...
int16_t printregs(void)
{
	...
	DO	aprintf("%s%8t%o%8t", p->rname, v=endhdr[p->roffs]);
