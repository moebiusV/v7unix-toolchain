...
int16_t numbval;	/* value of an input number */
char tokname[NAMESIZE];	/* input token name */

	/* storage of names */

char cnames[CNAMSZ];	/* place where token and nonterminal names are stored */
int16_t cnamsz = CNAMSZ;	/* size of cnames */
char * cnamp = cnames;	/* place where next name is to be put in */
int16_t ndefout = 3;  /* number of defined symbols output */

	/* storage of types */
int16_t ntypes;	/* number of types defined */
...
int16_t toklev[NTERMS];
int16_t nnonter = -1;
struct ntsymb nontrst[NNONTERM];
int16_t start;	/* start symbol */
...
int16_t mem0[MEMSIZE] ; /* production storage */
int16_t *mem = mem0;
int16_t nprod= 1;	/* number of productions */
int16_t *prdptr[NPROD];	/* pointers to descriptions of productions */
int16_t levprd[NPROD] ;	/* precedence levels for the productions */
...
{	int16_t i,j,lev,t, ty;
	...
	while( argc >= 2  && argv[1][0] == '-' ) {
		while( *++(argv[1]) ){
			...
			case 'V':
				...
				if( foutput == NULL ) error( "cannot open y.output" , 0);
				continue;
			case 'D':
			case 'd':
	...
	if( ftable == NULL ) error( "cannot open table file" , 0);

	ftemp = fopen( TEMPNAME, "w" );
	faction = fopen( ACTNAME, "w" );
	if( ftemp==NULL || faction==NULL ) error( "cannot open temp file" , 0);
	...
	for( t=gettok();  t!=MARK && t!= ENDFILE; ){
		...
		case START:
			if( (t=gettok()) != IDENTIFIER ){
				error( "bad %%start construction" , 0);
				}
		...
		case TYPEDEF:
			if( (t=gettok()) != TYPENAME ) error( "bad syntax in %%type" , 0);
			ty = numbval;
			for(;;){
				t = gettok();
				switch( t ){
	...
	*mem++ = 0;
	...
	if( t != C_IDENTIFIER ) error( "bad syntax on first rule" , 0);
	...
	while( t!=MARK && t!=ENDFILE ){
		...
		if( t == '|' ){
			...
			*mem = chfind(1,tokname);
			if( *mem < NTBASE ) error( "token illegal on LHS of grammar rule" , 0);
			++mem;
			}
		else error( "illegal rule: missing semicolon or | ?" , 0);
	...
	more_rule:
		...
		if( t == PREC ){
			if( gettok()!=IDENTIFIER) error( "illegal %%prec syntax" , 0);
			j = chfind(2,tokname);
		...
		*mem++ = -nprod;
		...
		if( ntypes && !(levprd[nprod]&ACTFLAG) && nontrst[*prdptr[nprod]-NTBASE].tvalue ){
			...
			register int16_t tempty;
			tempty = prdptr[nprod][1];
			if( tempty < 0 ) error( "must return a value, since LHS has a type" , 0);
			...
			if( tempty != nontrst[*prdptr[nprod]-NTBASE].tvalue ){
				error( "default action causes potential type clash" , 0);
				}
			}
...
/*	define s to be a terminal if t=0
	...
	if( s[0]==' ' && s[2]=='\0' ) /* single character literal */
		...
		if( s[3] == '\0' ){ /* single character escape sequence */
			...
			case '"': val = '"'; break;
			case '\\': val = '\\'; break;
			default: error( "invalid escape" , 0);
				}
			}
		else if( s[2] <= '7' && s[2]>='0' ){ /* \nnn sequence */
			...
			if( val == 0 ) error( "'\\000' is illegal" , 0);
			}
		}
	else {
		val = extval++;
...
int16_t defout(void){ /* write out the defines (at the end of the declaration section) */
...
char * cstash(char *s){
	...
	temp = cnamp;
	do {
		if( cnamp >= &cnames[cnamsz] ) error("too many characters in id's and literals" , 0);
		else *cnamp++ = *s;
		}  while ( *s++ );
	return( temp );
	}
...
int16_t gettok(void){
	register int16_t i, base;
	static int16_t peekline; /* number of '\n' seen in lookahead */
	register int16_t c, match, reserve;
...
begin:
	...
	case '<':  /* get, and look up, a type name (union member name) */
		...
		if( c != '>' ) error( "unterminated < ... > clause" , 0);
		tokname[i] = '\0';
		for( i=1; i<=ntypes; ++i ){
...
int16_t fdtype(int16_t t){ /* determine the type of a symbol */
	register int16_t v;
...
void cpyunion(void){
	...
	level = 0;
	for(;;){
		if( (c=getc(finput)) < 0 ) error( "EOF encountered while processing %%union" , 0);
		putc( c, ftable );
...
int16_t skipcom(void){ /* skip over comments */
	register int16_t c, i=0;  /* i is the number of lines skipped */

	/* skipcom is called after reading a / */

	if( getc(finput) != '*' ) error( "illegal comment" , 0);
	c = getc(finput);
	while( c != EOF ){
		while( c == '*' ){
	...
	error( "EOF inside comment" , 0);
	/* NOTREACHED */
	}

void cpyact(int16_t offset){ /* copy C action to the next ; or closing } */
	int16_t brac, c, match, j, s, tok;
...
case '$':
		...
		if( c == '<' ){ /* type description */
			ungetc( c, finput );
			if( gettok() != TYPENAME ) error( "bad syntax on $<ident> clause" , 0);
			tok = numbval;
			c = getc(finput);
			}
		if( c == '$' ){
...
case '"':	/* character string */
	...
	string:
		...
		while( c=getc(finput) ){
			...
			else if( c=='\n' ) error( "newline in string or char. const." , 0);
			putc( c , faction );
			}
		error( "EOF in string or character constant" , 0);

case EOF:
		error("action does not terminate" , 0);
