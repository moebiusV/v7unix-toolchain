...
int16_t lastred;  /* the number of the last reduction of a state */
int16_t defact[NSTATES];  /* the default actions of states */
...
int16_t output(void){ /* print the output for the states */
...
int16_t pkdebug = 0;
int16_t apack(int16_t *p, int16_t n){ /* pack state i from temp1 into amem */
	int16_t off;
	register int16_t *pp, *qq, *rr;
	int16_t *q, *r;
...
int16_t go2out(void){ /* output the gotos for the nontermninals */
	int16_t i, j, k, best, count, cbest, times;
...
int16_t g2debug = 0;
int16_t go2gen(int16_t c){ /* output the gotos for nonterminal c */
...
void precftn(int16_t r, int16_t t, int16_t s){ /* decide a shift/reduce conflict by precedence.
	/* r is a rule number, t a token number */
	/* the conflict is in state s */
	/* temp1[t] is changed to reflect the action */

	int16_t lp,lt, action;
...
void wract(int16_t i){ /* output state i */
	/* temp1 has the actions, lastred the default */
	int16_t p, p0, p1;
	int16_t ntimes, tred, count, j;
	int16_t flag;
...
void wrstate(int16_t i){ /* writes state i */
	register int16_t j0,j1;
...
int16_t wdef(char *s, int16_t n){ /* output a definition of s to the value n */
